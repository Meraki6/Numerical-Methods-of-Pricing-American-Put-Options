function price = CrankNicholson(S0, K, r, T, sigma, div, Smax, dS, dt, omega, tol)
M = round(Smax / dS);
dS = Smax / M;
N = round(T / dt);
dt = T / N;
% vector for Gauss-Seidel update
oldval = zeros(M-1, 1);
newval = zeros(M-1, 1);
...
alpha = 0.25 * dt * (sigma^2 * (veti.^2) - (r - div) * veti);
beta = - dt * 0.5 * (sigma^2 * (veti.^2) + r);
gamma = 0.25 * dt * (sigma^2 * (veti.^2) + (r - div) * veti);
...
% price = CrankNicholson(50,50,0.05,5/12,0.4,0,100,1,1/1200,1.8,0.001)
end
